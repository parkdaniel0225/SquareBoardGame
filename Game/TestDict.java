public class TestDict {
    ...
    public static void main(String[] args) {
        ...
        // Test 1: insert one Record item in the dictionary.
        // Should not throw an exception.

        if (test[1] || test[3] || test[6])
            try {
                dict.put(new Record("answer", 42, 1));
                System.out.println("   Test 1 succeeded");
            } catch (DuplicatedKeyException e) {
                System.out.println("***Test 1 failed");
            }

        // Test 2: try to insert another Record item with the same key.
        // Should throw an exception.
        if (test[2])
            try {
                dict.put(new Record("answer", 56, 1));
        ...
        if (test[7])
            try {
                for (i = 0; i < 10000; ++i) {
                    ...
                    for (j = 0; j < 5; ++j) s += s;
                    collisions += dict.put(new Record(s, i, i));
                }
        ...
        // Test 9: Remove the first 1000 Record items and verify that the rest
